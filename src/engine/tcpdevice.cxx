...
CTcpDevice::CTcpDevice(CDescriptor &descriptor)
{
    ...
    else
    {
        if (toupper(server[0]) == 'S')
        {
            ...
            {
                LOGINFO(gVerbose, "in single connection mode: all pending connections wait for current session to close\n");
...
bool CTcpDevice::Connect()
{
    ...
    if (!_connected)
    {
        ...
        else
        {
            ...
            {
                LOGERROR(1, "Cannot connect to server %s (error %d). %s\n", inet_ntoa(_serverAddr.sin_addr), errno, strerror(errno));
                close(_socketDesc);
                return false;
            }
            _connected = true;
...
bool CTcpDevice::InitClient(const char *serverAddress, const int serverPort, const char *clientAddress, const int clientPort)
{
    struct hostent *host;
...
void CTcpDevice::Init(const char *serverAddress, const int serverPort, const char *clientAddress, const int clientPort, const bool server)
{
    _opened = false;
    _connected = false;
    _socketDesc = -1;
